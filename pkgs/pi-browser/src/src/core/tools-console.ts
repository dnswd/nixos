...
export async function browser_console({
  ...
  if (enable === true) {
    const wasEnabled = isConsoleEnabled(target);
    await enableConsoleCapture(target);
    // Clear buffer on fresh enable (not re-enable), or if explicit clear requested
    if (!wasEnabled || clear) {
    ...
    return {
      ...
      count: 0,
      capturing: true,
      entriesCount: getConsoleBufferSize(target),
    };
  } else if (enable === false) {
    // Disable console capture
    await disableConsoleCapture(target);
    ...
    // No explicit enable/disable - just ensure capture is enabled for reading
    await enableConsoleCapture(target);
  }
